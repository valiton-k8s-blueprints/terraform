...
As this module is an addon, the STACKIT base module must also be installed. Important for the example is that you activate DNS for your SKE cluster with a domain provided by STACKIT.
You also need a STACKIT service account (for the Cert Manager webhook to perform the DNS01 challenge) with the authorization for dns.admin or dns.editor. The Terraform bootstrapper will then create an access token for the service account, which will be used by the STACKIT Cert Manager webhook.
